...
<body class="font-['Nunito',sans-serif] antialiased bg-gray-50 text-slate-800">
    ...
    <aside id="adminSidebar" class="fixed top-0 left-0 z-50 w-64 h-screen bg-emerald-900 transform -translate-x-full lg:translate-x-0 transition-transform duration-300 flex flex-col">
        ...
        <div class="flex-1 overflow-y-auto py-4 px-3 space-y-1">
            ...
            {{-- Properties --}}
            <div class="sidebar-group">
                <button onclick="toggleMenu('menu-properties')" class="sidebar-link w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-emerald-100 text-sm font-medium {{ request()->routeIs('admin.properties*') ? 'active' : '' }}">
                    ...
                    <svg class="w-4 h-4 ml-auto transition-transform" id="arrow-properties" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                </button>
                <div id="menu-properties" class="sidebar-submenu pl-11 space-y-0.5 {{ request()->routeIs('admin.properties*') ? 'open' : '' }}">
                    <a href="#" class="block py-1.5 text-xs text-emerald-200/70 hover:text-white transition-colors">All Properties</a>
                    <a href="#" class="block py-1.5 text-xs text-emerald-200/70 hover:text-white transition-colors">Pending Approval</a>
                    <a href="#" class="block py-1.5 text-xs text-emerald-200/70 hover:text-white transition-colors">Featured</a>
                </div>
            </div>

            {{-- Users --}}
            <div class="sidebar-group">
                <button onclick="toggleMenu('menu-users')" class="sidebar-link w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-emerald-100 text-sm font-medium {{ request()->routeIs('admin.users*') ? 'active' : '' }}">
                    <svg class="w-5 h-5 text-gold-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/></svg>
                    <span>Users</span>
                    <svg class="w-4 h-4 ml-auto transition-transform" id="arrow-users" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                </button>
                <div id="menu-users" class="sidebar-submenu pl-11 space-y-0.5 {{ request()->routeIs('admin.users*') ? 'open' : '' }}">
                    <a href="#" class="block py-1.5 text-xs text-emerald-200/70 hover:text-white transition-colors">All Users</a>
                    <a href="#" class="block py-1.5 text-xs text-emerald-200/70 hover:text-white transition-colors">Landlords</a>
                    <a href="#" class="block py-1.5 text-xs text-emerald-200/70 hover:text-white transition-colors">Tenants</a>
                </div>
            </div>

            {{-- Listings --}}
            <div class="sidebar-group">
                <a href="#" class="sidebar-link w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-emerald-100 text-sm font-medium {{ request()->routeIs('admin.listings*') ? 'active' : '' }}">
            ...
            {{-- Reports --}}
            <div class="sidebar-group">
                <a href="#" class="sidebar-link w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-emerald-100 text-sm font-medium {{ request()->routeIs('admin.reports*') ? 'active' : '' }}">
            ...
            {{-- Settings --}}
            <div class="sidebar-group">
                <a href="#" class="sidebar-link w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-emerald-100 text-sm font-medium {{ request()->routeIs('admin.settings*') ? 'active' : '' }}">
