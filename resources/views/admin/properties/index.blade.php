...
<div class="mb-4 flex flex-wrap items-center justify-between gap-3">
    <form method="GET" class="flex flex-wrap items-center gap-3">
        ...
        <select name="type" class="px-3 py-2 text-sm rounded-lg border border-gray-200 focus:border-emerald-500 outline-none">
            ...
            @foreach(['apartment'=>'Apartment','house'=>'House','commercial'=>'Commercial','land'=>'Land','studio'=>'Studio','maisonette'=>'Maisonette','hotel'=>'Hotel','guest_house'=>'Guest House','multi_unit'=>'Multi-Unit'] as $val=>$label)
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm" id="propertiesTable">
            <thead><tr class="text-left text-xs text-gray-500 bg-gray-50/50">
                ...
                <th class="px-5 py-3 font-medium">Units</th>
            ...
            <tbody id="propertiesBody">
                ...
                <tr class="border-t border-gray-100 hover:bg-gray-50/50 transition-colors" id="row-{{ $property->id }}">
                    ...
                    <td class="px-5 py-3">
                        ...
                        @else
                            <span class="text-[10px] text-gray-400">Single</span>
                        @endif
                    </td>
                    <td class="px-5 py-3 text-xs font-semibold text-gray-900">
                        @if($property->price_min && $property->price_max)
                            {{ number_format($property->price_min) }} - {{ number_format($property->price_max) }}
                        @else
                            {{ number_format($property->price) }}
                        @endif
                        <span class="text-[9px] text-gray-400 font-normal">/{{ $property->rental_period }}</span>
                    </td>
...
<div id="addPropertyModal" class="fixed inset-0 bg-black/50 z-50 hidden items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl max-w-3xl w-full overflow-hidden max-h-[90vh] overflow-y-auto">
        <div class="bg-gradient-to-r from-emerald-600 to-emerald-800 px-6 py-4 flex items-center justify-between sticky top-0 z-10">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-emerald-100/80">Create a powerful property listing</p>
                </div>
            </div>
        ...
        <form id="addPropertyForm" class="p-6 space-y-5">
            @csrf
            {{-- Section: Basic Info --}}
            <div>
                <h4 class="text-xs font-bold text-gray-400 uppercase tracking-wide mb-3 flex items-center gap-2">
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-1h1v1a1 1 0 011-1h1v1a1 1 0 01-1 1h-1a1 1 0 01-1-1z M13 16V8a1 1 0 011-1h1v1a1 1 0 01-1 1h-1"/></svg>
                    Basic Information
                </h4>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="user_id" required class="w-full px-3 py-2.5 text-sm rounded-lg border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all" onchange="updateLandlordPhone(this)">
                            <option value="">Select landlord...</option>
                            @foreach($landlords as $landlord)
                            <option value="{{ $landlord->id }}" data-phone="{{ $landlord->phone ?? '' }}">{{ $landlord->name }} ({{ $landlord->email }})</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-gray-700 mb-1.5">Property Type <span class="text-red-500">*</span></label>
                        <select name="property_type" required id="propertyTypeSelect" class="w-full px-3 py-2.5 text-sm rounded-lg border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all" onchange="onPropertyTypeChange()">
                            @foreach(['apartment'=>'Apartment','house'=>'House','commercial'=>'Commercial','land'=>'Land','studio'=>'Studio','maisonette'=>'Maisonette','hotel'=>'Hotel','guest_house'=>'Guest House','multi_unit'=>'Multi-Unit Building'] as $val=>$label)
            ...
            <div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <label class="cursor-pointer">
                        <input type="radio" name="listing_type" value="single" checked onchange="onListingTypeChange()" class="peer sr-only">
                        <div class="border-2 rounded-xl p-4 peer-checked:border-emerald-500 peer-checked:bg-emerald-50 transition-all">
                            <div class="flex items-center gap-2 mb-1">
                                ...
                                <span class="text-sm font-bold text-gray-900">Single Unit</span>
                            </div>
                            <p class="text-[10px] text-gray-500">One property, one unit (house, apartment, etc.)</p>
                        </div>
                    </label>
                    <label class="cursor-pointer">
                        <input type="radio" name="listing_type" value="multi_unit" onchange="onListingTypeChange()" class="peer sr-only">
                        <div class="border-2 rounded-xl p-4 peer-checked:border-emerald-500 peer-checked:bg-emerald-50 transition-all">
                            <div class="flex items-center gap-2 mb-1">
                                ...
                                <span class="text-sm font-bold text-gray-900">Multi-Unit</span>
                            </div>
                            <p class="text-[10px] text-gray-500">Hotel, guest house, apartments with many rooms</p>
                        </div>
                    </label>
                </div>
                <div id="totalUnitsField" class="hidden mt-3">
                    <label class="block text-xs font-semibold text-gray-700 mb-1.5">Total Number of Units</label>
                    <input type="number" name="total_units" min="1" value="2" placeholder="e.g. 10" class="w-full px-3 py-2.5 text-sm rounded-lg border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all">
                    <p class="text-[10px] text-gray-400 mt-1">You can add individual unit details after creating the property</p>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="number" name="price" required min="0" step="1000" placeholder="e.g. 500000" class="w-full px-3 py-2.5 text-sm rounded-lg border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-gray-700 mb-1.5">Min Price (TZS)</label>
                        <input type="number" name="price_min" min="0" step="1000" placeholder="For range" class="w-full px-3 py-2.5 text-sm rounded-lg border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-gray-700 mb-1.5">Max Price (TZS)</label>
                        <input type="number" name="price_max" min="0" step="1000" placeholder="For range" class="w-full px-3 py-2.5 text-sm rounded-lg border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-gray-700 mb-1.5">Rental Period</label>
                        <select name="rental_period" class="w-full px-3 py-2.5 text-sm rounded-lg border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <div class="flex gap-2">
                            <input type="text" name="contact_phone" id="contactPhoneInput" placeholder="Custom phone number" class="flex-1 px-3 py-2.5 text-sm rounded-lg border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all">
                            <button type="button" onclick="useLandlordPhone()" class="px-3 py-2.5 text-xs font-medium text-emerald-600 bg-emerald-50 rounded-lg hover:bg-emerald-100 transition-all whitespace-nowrap flex items-center gap-1">
                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                                Use Landlord's
                            </button>
                        </div>
                        <p class="text-[10px] text-gray-400 mt-1">Leave empty to use landlord's phone automatically</p>
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-gray-700 mb-1.5">Street</label>
                        <input type="text" name="street" placeholder="e.g. Ali Hassan Mwinyi Rd" class="w-full px-3 py-2.5 text-sm rounded-lg border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all">
                    </div>
                </div>
            </div>

            {{-- Section: Details --}}
            <div id="singleUnitDetails">
                <h4 class="text-xs font-bold text-gray-400 uppercase tracking-wide mb-3 flex items-center gap-2">
            ...
            <div>
                ...
                <textarea name="description" rows="3" placeholder="Describe the property, amenities, nearby facilities..." class="w-full px-3 py-2.5 text-sm rounded-lg border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all"></textarea>
            </div>
...
function onListingTypeChange() {
    ...
    const totalUnitsField = document.getElementById('totalUnitsField');
    const singleDetails = document.getElementById('singleUnitDetails');
    if (listingType === 'multi_unit') {
        totalUnitsField.classList.remove('hidden');
        singleDetails.classList.add('hidden');
    } else {
        totalUnitsField.classList.add('hidden');
...
function onPropertyTypeChange() {
    ...
    if (type === 'hotel' || type === 'guest_house' || type === 'multi_unit') {
        const multiRadio = document.querySelector('input[name="listing_type"][value="multi_unit"]');
        multiRadio.checked = true;
        onListingTypeChange();
    }
}

function updateLandlordPhone(select) {
    const phone = select.options[select.selectedIndex]?.dataset?.phone || '';
    const phoneInput = document.getElementById('contactPhoneInput');
    if (phoneInput.value === '') {
        phoneInput.placeholder = phone ? 'Landlord: ' + phone : 'Custom phone number';
    }
}

function useLandlordPhone() {
    const select = document.querySelector('select[name="user_id"]');
    const phone = select.options[select.selectedIndex]?.dataset?.phone || '';
    if (phone) {
        document.getElementById('contactPhoneInput').value = phone;
        showToast('Landlord phone filled: ' + phone);
    } else {
        showToast('This landlord has no phone number', 'error');
    }
}
